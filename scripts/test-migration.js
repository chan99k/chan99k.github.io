...
class MigrationTester {
  ...
  async test() {
    ...
    try {
      ...
      if (failedTests.length === 0) {
        ...
        console.log(`📊 ${this.testResults.length} tests completed successfully`);
      } else {
  ...
  async testBlogPosts() {
    ...
    try {
      ...
      this.addTestResult('blog', 'general', false, `Blog testing failed: ${error.message}`);
    }
  }
  ...
  async testBlogPost(filePath) {
    ...
    try {
      ...
      if (missingFields.length > 0) {
        this.addTestResult('blog', relativePath, false, `Missing fields: ${missingFields.join(', ')}`);
        return;
      }
      
      // Test data types
      if (!Array.isArray(frontmatter.tags)) {
        this.addTestResult('blog', relativePath, false, 'Tags must be an array');
        return;
      }
      ...
      if (frontmatter.isProblemSolution && frontmatter.problemSolutionMeta) {
        ...
        if (!meta.problem || !meta.solution || !Array.isArray(meta.technologies)) {
          this.addTestResult('blog', relativePath, false, 'Invalid problem-solution metadata');
          return;
        }
      }
      ...
      this.addTestResult('blog', relativePath, true, 'Blog post structure valid');
      
    } catch (error) {
      this.addTestResult('blog', relativePath, false, `File parsing failed: ${error.message}`);
    }
  }
  ...
  async testPortfolioContent() {
    ...
    try {
      ...
      if (!frontmatter.personalInfo || !frontmatter.personalInfo.name) {
        this.addTestResult('portfolio', 'portfolio.md', false, 'Missing or invalid personalInfo');
        return;
      }
      ...
      for (const field of arrayFields) {
        if (frontmatter[field] && !Array.isArray(frontmatter[field])) {
          this.addTestResult('portfolio', 'portfolio.md', false, `${field} must be an array`);
          return;
        }
      }
      ...
      if (frontmatter.projects) {
        for (const project of frontmatter.projects) {
          if (!project.id || !project.title || !Array.isArray(project.techStack)) {
            this.addTestResult('portfolio', 'portfolio.md', false, 'Invalid project structure');
            return;
          }
          ...
          if (project.problems) {
            for (const problem of project.problems) {
              if (!problem.id || !problem.title || !problem.problem || !problem.solution) {
                this.addTestResult('portfolio', 'portfolio.md', false, 'Invalid problem structure');
                return;
              }
            }
          }
      ...
      this.addTestResult('portfolio', 'portfolio.md', true, 'Portfolio structure valid');
      console.log('✅ Portfolio content tested');
      
    } catch (error) {
      this.addTestResult('portfolio', 'portfolio.md', false, `Portfolio testing failed: ${error.message}`);
    }
  }
  ...
  async testRestaurantReviews() {
    ...
    try {
      ...
      this.addTestResult('reviews', 'general', false, `Reviews testing failed: ${error.message}`);
    }
  }
  ...
  async testRestaurantReview(filePath) {
    ...
    try {
      ...
      if (missingFields.length > 0) {
        this.addTestResult('reviews', relativePath, false, `Missing fields: ${missingFields.join(', ')}`);
        return;
      }
      
      // Test location structure
      if (!frontmatter.location.address || !frontmatter.location.coordinates) {
        this.addTestResult('reviews', relativePath, false, 'Invalid location structure');
        return;
      }
      ...
      if (isNaN(rating) || rating < 0 || rating > 5) {
        this.addTestResult('reviews', relativePath, false, 'Invalid rating (must be 0-5)');
        return;
      }
      
      // Test arrays
      if (frontmatter.tags && !Array.isArray(frontmatter.tags)) {
        this.addTestResult('reviews', relativePath, false, 'Tags must be an array');
        return;
      }
      
      if (frontmatter.images && !Array.isArray(frontmatter.images)) {
        this.addTestResult('reviews', relativePath, false, 'Images must be an array');
        return;
      }
      
      this.addTestResult('reviews', relativePath, true, 'Restaurant review structure valid');
      
    } catch (error) {
      this.addTestResult('reviews', relativePath, false, `File parsing failed: ${error.message}`);
    }
  }
