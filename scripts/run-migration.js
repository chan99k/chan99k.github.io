...
class MigrationRunner {
  ...
  async generateFinalReport() {
    ...
    try {
      const migrationReport = await fs.readFile('migration-report.json', 'utf8');
    ...
    try {
      const validationReport = await fs.readFile('validation-report.json', 'utf8');
    ...
    try {
      const imageReport = await fs.readFile('image-optimization-report.json', 'utf8');
    ...
    await fs.writeFile('final-migration-report.json', JSON.stringify(finalReport, null, 2));
  ...
  async generateErrorReport(error) {
    ...
    await fs.writeFile('migration-error-report.json', JSON.stringify(errorReport, null, 2));
  }
    ...
    return [
      {
        ...
        files: ['migration-report.json', 'validation-report.json', 'image-optimization-report.json']
      },
      {
      ...
      },
      {
        ...
        command: 'git add . && git commit -m "Complete content migration" && git push'
      },
      {
    ...
    return {
      commonIssues: [
        ...
        },
        {
          ...
          solution: 'Check validation-report.json for specific issues and fix frontmatter'
        },
        {
          issue: 'Build failures',
          solution: 'Check that all content files have valid frontmatter and no syntax errors'
        }
      ],
      rollbackProcedure: [
  ...
  printSummary(report) {
    ...
    if (report.detailedReports.migration?.summary) {
      ...
      console.log(`📝 Content Files: ${migrationSummary.successful}/${migrationSummary.total} migrated`);
    }
    
    if (report.detailedReports.validation?.summary) {
      const validationSummary = report.detailedReports.validation.summary;
      console.log(`🔍 Validation: ${validationSummary.errors} errors, ${validationSummary.warnings} warnings`);
    }
    
    if (report.detailedReports.imageOptimization?.summary) {
      const imageSummary = report.detailedReports.imageOptimization.summary;
      console.log(`🖼️  Images: ${imageSummary.successful}/${imageSummary.total} optimized`);
    }
  }
...
if (require.main === module) {
  ...
  process.on('SIGINT', async () => {
    ...
    await runner.generateErrorReport(new Error('Migration interrupted by user'));
    process.exit(1);
  });
