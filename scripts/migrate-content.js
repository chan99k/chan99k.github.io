...
class ContentMigrator {
  ...
  validatePortfolioStructure(frontmatter) {
    ...
    // Ensure arrays exist
    validated.experience = Array.isArray(validated.experience)
      ? validated.experience
      : [];
    validated.projects = Array.isArray(validated.projects)
      ? validated.projects
      : [];
    validated.certifications = Array.isArray(validated.certifications)
      ? validated.certifications
      : [];
    validated.education = Array.isArray(validated.education)
      ? validated.education
      : [];
    ...
    validated.projects = validated.projects.map(project => ({
      ...
      problems: Array.isArray(project.problems)
        ? project.problems.map(problem => ({
            id: problem.id || `problem-${Date.now()}`,
            title: problem.title || 'Untitled Problem',
            problem: problem.problem || '',
            solution: problem.solution || '',
            technologies: Array.isArray(problem.technologies)
  ...
  async migrateRestaurantReview(filePath) {
    ...
    try {
      ...
      const validatedFrontmatter =
        this.validateRestaurantReviewStructure(frontmatter);
  ...
  validateRestaurantReviewStructure(frontmatter) {
    ...
    validated.visitDate =
      validated.visitDate || new Date().toISOString().split('T')[0];
  ...
  async generateMigrationReport() {
    const report = {
      ...
      summary: {
        ...
        successful: this.migrationLog.filter(log => log.status === 'success')
          .length,
