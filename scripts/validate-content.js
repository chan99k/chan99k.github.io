...
class ContentValidator {
    ...
    try {
      ...
      if (this.validationErrors.length === 0) {
        ...
        if (this.validationWarnings.length > 0) {
          console.log(
            `⚠️  ${this.validationWarnings.length} warnings found (see validation-report.json)`
          );
        }
      } else {
        console.log(
          `❌ ${this.validationErrors.length} validation errors found`
        );
  ...
  async validateBlogPosts() {
    ...
    try {
      ...
      this.addError(
        'blog',
        'general',
        `Blog validation failed: ${error.message}`
      );
    }
  }
  ...
  async validateBlogPost(filePath) {
    try {
      ...
      if (frontmatter.isProblemSolution) {
        if (!frontmatter.problemSolutionMeta) {
          this.addWarning(
            'blog',
            relativePath,
            'Problem-solution post missing problemSolutionMeta'
          );
        } else {
          ...
          if (!meta.problem || !meta.solution) {
            this.addError(
              'blog',
              relativePath,
              'Problem-solution meta must include problem and solution'
            );
          }
          if (!Array.isArray(meta.technologies)) {
            this.addError(
              'blog',
              relativePath,
              'Problem-solution technologies must be an array'
            );
          }
        }
      }
      ...
      this.validateImageReferencesInContent(
        markdownContent,
        relativePath,
        'blog'
      );
        ...
        for (let i = 0; i < frontmatter.projects.length; i++) {
          ...
          if (project.techStack && !Array.isArray(project.techStack)) {
            this.addError(
              'portfolio',
              'portfolio.md',
              `${projectPath}.techStack must be an array`
            );
          }
          ...
          if (project.problems) {
            if (!Array.isArray(project.problems)) {
              this.addError(
                'portfolio',
                'portfolio.md',
                `${projectPath}.problems must be an array`
              );
            } else {
              for (let j = 0; j < project.problems.length; j++) {
                ...
                if (
                  ...
                  !Array.isArray(problem.technologies)
                ) {
                  this.addError(
                    'portfolio',
                    'portfolio.md',
                    `${problemPath}.technologies must be an array`
                  );
                }
              }
            }
  ...
  async validateRestaurantReviews() {
    ...
    try {
      ...
      this.addError(
        'reviews',
        'general',
        `Reviews validation failed: ${error.message}`
      );
    }
  }
    ...
    try {
      ...
      if (frontmatter.priceRange !== undefined) {
        ...
        if (isNaN(priceRange) || priceRange < 1 || priceRange > 5) {
          this.addError(
            'reviews',
            relativePath,
            'Price range must be a number between 1 and 5'
          );
        }
      }
      ...
      if (frontmatter.visitDate && !this.isValidDate(frontmatter.visitDate)) {
        this.addError(
          'reviews',
          relativePath,
          'Invalid visitDate format (expected YYYY-MM-DD)'
        );
      }
      ...
      if (frontmatter.mapLinks) {
        ...
        for (const provider of mapProviders) {
          if (
            frontmatter.mapLinks[provider] &&
            !this.isValidUrl(frontmatter.mapLinks[provider])
          ) {
            this.addWarning(
              'reviews',
              relativePath,
              `Invalid ${provider} map URL`
            );
          }
        }
      }
    ...
    for (const match of matches) {
      ...
      if (!imagePath.startsWith('/images/')) {
        this.addWarning(
          type,
          filePath,
          `Image path should start with /images/: ${imagePath}`
        );
      }
    }
  }
