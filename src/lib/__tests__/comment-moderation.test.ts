...
describe('Comment Moderation', () => {
  describe('spamDetection', () => {
    ...
    it('should detect excessive links', () => {
      const spamText = 'Visit http://site1.com and http://site2.com and http://site3.com and http://site4.com';
      const result = spamDetection.analyzeForSpam(spamText);
      
      expect(result.isSpam).toBe(true);
      expect(result.reasons.some(reason => reason.includes('Excessive links'))).toBe(true);
    });

    it('should allow legitimate content', () => {
      const legitimateText = 'This is a great blog post! Thanks for sharing your insights.';
  ...
  describe('contentFilter', () => {
    ...
    it('should allow appropriate content', () => {
      const appropriateText = 'This is a wonderful article about web development.';
  ...
  describe('rateLimiting', () => {
    ...
    it('should block submissions when rate limit exceeded', () => {
      ...
      for (let i = 0; i < maxSubmissions; i++) {
        expect(rateLimiting.isRateLimited(userId, maxSubmissions, 300000)).toBe(false);
        rateLimiting.recordSubmission(userId);
      }
      
      // 다음 제출은 차단되어야 함
      expect(rateLimiting.isRateLimited(userId, maxSubmissions, 300000)).toBe(true);
    });
    ...
    it('should reset rate limit after time window', () => {
      ...
      expect(rateLimiting.isRateLimited(userId, maxSubmissions, timeWindow)).toBe(true);
      
      // 시간 윈도우가 지날 때까지 대기
      return new Promise(resolve => {
        setTimeout(() => {
          expect(rateLimiting.isRateLimited(userId, maxSubmissions, timeWindow)).toBe(false);
          resolve(void 0);
        }, timeWindow + 10);
      });
    });
  ...
  describe('moderateComment', () => {
    ...
    it('should allow legitimate comments', () => {
      const legitimateComment = 'Great article! I learned a lot about React hooks.';
    ...
    it('should block spam comments', () => {
      const spamComment = 'Click here to win free money! Visit now! http://suspicious.tk';
    ...
    it('should provide detailed analysis', () => {
      const comment = 'Check out http://site1.com and http://site2.com and http://site3.com and http://site4.com';
