...
jest.mock('../content', () => ({
  ...
  getRestaurantReviews: jest.fn(() => Promise.resolve([
    {
      id: 'test-restaurant',
...
describe('RSS Feed and Sitemap Generation', () => {
  describe('RSS Feed', () => {
    it('should generate valid RSS XML', async () => {
      ...
      expect(rssContent).toContain('<title><![CDATA[Personal Website]]></title>');
    ...
    it('should include proper RSS metadata', async () => {
      ...
      expect(rssContent).toContain('<generator>Next.js Personal Website</generator>');
      expect(rssContent).toContain('<atom:link href="http://localhost:3000/rss.xml"');
  ...
  describe('Sitemap', () => {
    ...
    it('should have proper sitemap structure', async () => {
      ...
      sitemapData.forEach(item => {
        ...
        expect(['always', 'hourly', 'daily', 'weekly', 'monthly', 'yearly', 'never']).toContain(item.changeFrequency);
