...
export class ContentLoader {
  ...
  static parseContentFile(filePath: string): ContentLoadResult<ContentFile> {
    try {
      ...
      return {
        success: false,
        errors: [`Error parsing file ${filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`],
      };
    }
  }
    ...
    for (const filePath of files) {
      ...
      try {
        const post: BlogPost = {
          ...
          description: (validatedData?.description as string) || generateExcerpt(content),
          ...
          isProblemSolution: (validatedData?.isProblemSolution as boolean) || false,
        ...
        errors.push(`Error creating blog post object for ${filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  ...
  static async loadBlogPost(slug: string): Promise<ContentLoadResult<{ post: BlogPost; content: string }>> {
    ...
    try {
      ...
      return {
        success: false,
        errors: [`Error creating blog post object: ${error instanceof Error ? error.message : 'Unknown error'}`],
      };
    }
  }
    ...
    for (const filePath of files) {
      ...
      try {
        const review: RestaurantReview = {
          ...
          location: (validatedData?.location as RestaurantReview['location']) || { 
            address: '', 
            coordinates: { lat: 0, lng: 0 }, 
            region: '' 
          },
          rating: (validatedData?.rating as number) || 0,
          visitDate: (validatedData?.visitDate as string) || new Date().toISOString(),
          cuisine: (validatedData?.cuisine as RestaurantReview['cuisine']) || 'other',
          priceRange: (validatedData?.priceRange as RestaurantReview['priceRange']) || 1,
          images: (validatedData?.images as RestaurantReview['images']) || [],
          review: content,
          tags: (validatedData?.tags as string[]) || [],
          mapLinks: (validatedData?.mapLinks as RestaurantReview['mapLinks']) || { naver: '', kakao: '', google: '' },
        };

        reviews.push(review);
      } catch (error) {
        errors.push(`Error creating restaurant review object for ${filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  ...
  static async loadPortfolioData(): Promise<ContentLoadResult<PortfolioData>> {
    ...
    const warnings: string[] = [];
    if (!validation.success) {
      warnings.push(`Portfolio frontmatter validation failed for ${portfolioPath}`);
      validation.errors?.forEach(error => warnings.push(`  - ${error}`));
    }
    ...
    try {
      const portfolioData: PortfolioData = {
        personalInfo: (validatedData?.personalInfo as PortfolioData['personalInfo']) || {
          name: 'Chan99K',
        ...
        experience: (validatedData?.experience as PortfolioData['experience']) || [],
        projects: (validatedData?.projects as PortfolioData['projects']) || [],
        certifications: (validatedData?.certifications as PortfolioData['certifications']) || [],
        education: (validatedData?.education as PortfolioData['education']) || [],
      };
      ...
      return {
        success: false,
        errors: [`Error creating portfolio data object: ${error instanceof Error ? error.message : 'Unknown error'}`],
      };
    }
  }
  ...
  static async getContentStats(): Promise<{
    ...
    return {
      ...
      totalFiles: (blogResult.data?.length || 0) + (reviewsResult.data?.length || 0) + 1, // +1 for portfolio
    };
  }
}
