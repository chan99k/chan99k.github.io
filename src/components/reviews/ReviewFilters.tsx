...
export function ReviewFilters({
  ...
  const cuisineOptions = Array.from(
    new Set(reviews.map(r => r.cuisine))
  ).sort();
  const regionOptions = Array.from(
    new Set(reviews.map(r => r.location.region))
  ).sort();
  ...
  const applyFilters = (newFilters: FilterState) => {
    ...
    if (newFilters.cuisine !== 'all') {
      filtered = filtered.filter(
        review => review.cuisine === newFilters.cuisine
      );
    }

    if (newFilters.rating !== 'all') {
      filtered = filtered.filter(
        review => review.rating >= (newFilters.rating as number)
      );
    }

    if (newFilters.region !== 'all') {
      filtered = filtered.filter(
        review => review.location.region === newFilters.region
      );
    }

    if (newFilters.priceRange !== 'all') {
      filtered = filtered.filter(
        review => review.priceRange === (newFilters.priceRange as number)
      );
    }
              ...
              >
                ...
                {ratingOptions.map(rating => (
                  <option key={rating} value={rating}>
                    <div className='flex items-center'>{rating}점 이상</div>
                  </option>
                ))}
              </select>
            </div>
