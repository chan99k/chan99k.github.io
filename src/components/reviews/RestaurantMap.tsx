...
import { generateEmbeddedMapUrls, getBestMapProvider, isInSouthKorea } from '@/lib/map-utils';
...
export function RestaurantMap({ 
  ...
  const [mapProvider, setMapProvider] = useState<'naver' | 'kakao' | 'google'>('naver');
  ...
  const getMapCenter = () => {
    if (restaurants.length === 0) return { lat: 37.5665, lng: 126.9780 }; // Seoul default
    
    if (selectedRestaurant) {
      return selectedRestaurant.location.coordinates;
    }

    const avgLat = restaurants.reduce((sum, r) => sum + r.location.coordinates.lat, 0) / restaurants.length;
    const avgLng = restaurants.reduce((sum, r) => sum + r.location.coordinates.lng, 0) / restaurants.length;
  ...
  const getStaticMapUrl = () => {
    switch (mapProvider) {
      ...
      case 'google':
        return process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY ? generateGoogleStaticMap() : null;
      default:
        return null;
    }
  };
        ...
        <div className={`relative ${height} bg-gray-100 dark:bg-gray-700`}>
          {mapError ? (
            <div className="h-full flex items-center justify-center">
              <p className="text-gray-500 dark:text-gray-400 text-sm">{mapError}</p>
            </div>
          ) : (
            <img
              src={getStaticMapUrl() || ''}
            ...
            <div className="absolute bottom-4 left-4 right-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3">
              ...
              <div className="flex items-center mt-2">
                <div className="flex text-yellow-400 text-xs">
                  {[...Array(5)].map((_, i) => (
                    <span key={i} className={i < selectedRestaurant.rating ? '★' : '☆'}>
                      ★
                    </span>
                  ))}
                </div>
