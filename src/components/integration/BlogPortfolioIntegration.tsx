    ...
    <section className='mb-16'>
      <div className='mb-8'>
        ...
        <p className='text-muted-foreground'>
          Connecting detailed blog posts with portfolio projects and problem
          solutions
        </p>
      </div>
                      ...
                      <div className='grid grid-cols-1 gap-2 text-xs'>
                        ...
                        {post.problemSolutionMeta.technologies.length > 0 && (
                          <div className='flex flex-wrap gap-1'>
                            {post.problemSolutionMeta.technologies.map(
                              (tech, index) => (
                                <span
                                  key={index}
                  ...
                  >
                    ...
                    <div className='flex items-center justify-between text-xs text-muted-foreground'>
                      <div className='flex items-center space-x-4'>
                        <span>
                          Project:{' '}
                          {projects.find(p => p.id === problem.projectId)
                            ?.title || 'Unknown'}
                        </span>
            ...
            <div className='text-2xl font-bold text-foreground'>
              {
                Array.from(
                  new Set(
                    blogPosts
                      .filter(p => p.relatedProject)
                      .map(p => p.relatedProject)
                  )
                ).length
              }
            </div>
